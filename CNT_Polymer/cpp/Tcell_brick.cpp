...
Tcell_brick::Tcell_brick(int cell_id, Tnode* (&node_ptr)[8], TMAT_base* mat)
{
	_id = cell_id;
	for (int i = 0; i < 8; i++)
	{
	...
	_soundspeed = _mat_ptr->G_soundspeed();
	_strain_energy = 0;
	return;
}
double Tcell_brick::calculate_characteristic_length()
{
	...
	for (int i = 0; i < 6; i++)
	{
		...
		//for (int j = 0; j < 4; j++)
		//{
		//	p[j] = _node_ptr[cell_face[i][j]]->_position;
		//}
		//double s = area(p[0], p[1], p[2]) + area(p[0], p[2], p[3]);
...
{
	double mass = _volume * _mat_ptr->G_density();
	for (int i = 0; i < 8; i++)
	{
...
void Tcell_brick::calculate_corner_force(double dt)
{
	...
	//Update the stress
	_mat_ptr->update_state(de, vort, dt);
	...
	double sig[6];
	_mat_ptr->G_stress(sig);
	double sxx = sig[0], syy = sig[1], szz = sig[2], sxy = sig[3], sxz = sig[4], syz = sig[5];
	for (int i = 0; i < 8; i++)
	{
		_corner_force[i].x = - (sxx*DN[i].x + sxy * DN[i].y + sxz * DN[i].z)*_volume;
		_corner_force[i].y = - (sxy*DN[i].x + syy * DN[i].y + syz * DN[i].z)*_volume;
		_corner_force[i].z = - (sxz*DN[i].x + syz * DN[i].y + szz * DN[i].z)*_volume;
	}
	vec3D HG_force[8];
	calculate_hourglass_viscosity_force(HG_force);
	for (int i = 0; i < 8; i++)
	{
		_corner_force[i] = _corner_force[i] + HG_force[i];
	}
	return;
}
...
void Tcell_brick::update_strain_energy(double dt)
{
	...
	for (int i = 0; i < 4; i++)
	{
		de = de + _corner_force[i] * _node_ptr[i]->_velocity;
