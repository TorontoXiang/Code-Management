...
double Tcell_pure_fluid_MPM::calculate_time_step_particle()
{
	if (!_is_empty)
	{
...
void Tcell_pure_fluid_MPM::distribute_particle_force(TMPM_particle* particle_ptr)
{
...
void Tcell_pure_fluid_MPM::update_particle_DOF(TMPM_particle* particle_ptr,double dt)
{
...
void Tcell_pure_fluid_MPM::update_particle_stress(TMPM_particle* particle_ptr,double dt)
{
	//Calculate the shape function at the particle
	vec3D standard_coordinate=particle_ptr->_standard_coordinate;
	vec3D GNshape[8];
...
void Tcell_pure_fluid_MPM::distribute_particle_variables(TMPM_particle* particle_ptr,vec3D interval)
{
	_is_empty=false;
...
vec3D Tcell_pure_fluid_MPM::calculate_standard_coordinate(vec3D& particle_coordinate,vec3D interval)
{
...
void Tcell_pure_fluid_MPM::reset_state_after_remapping()
{
	Tcell_pure_fluid::reset_state_after_remapping();
	_is_empty=true;
	return;
}
void Tcell_pure_fluid_MPM::assemble_corner_force()
{
	for (int i = 0; i < 8; i++)
	{
